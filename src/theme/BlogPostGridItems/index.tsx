...
export default function BlogPostGridItems({ items }: BlogPostItemsProps): JSX.Element {
  ...
  const data = items.map(({ content: BlogPostContent }) => {
    ...
    const image = assets?.image ?? (frontMatter.image ? withBaseUrl(frontMatter.image) : undefined)
      ...
      {data.map((item, idx) => (
        ...
        >
          <Link href={item.link} className="hover:no-underline">
            ...
            <Card
              className={cn('relative bg-blog', item.sticky && styles.blogSticky)}
              header={
                item.image && (
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        'relative z-20 h-full w-full overflow-hidden rounded-lg border border-transparent bg-background group-hover:border-slate-700 dark:border-white/[0.2]',
        className,
      )}
    >
      {header}
